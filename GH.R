...
# See also: http://rpubs.com/FJRubio/GHGH
#           http://rpubs.com/FJRubio/GHPH
...
ae <- 1.75; be <- 0.6; ce <- 2.5; beta <- c(0.05,0.2,0.25); betaH <- c(0.1,0.1,0.1); 
true.par <- c(ae,be,ce,betaH,beta)
true.parc <- c(ae,be,ce,beta)

# read the data (available at: )
data <- read.table("dataGH.txt")
colnames(data)

# create the variables for the models
x <- as.matrix(data[,1:3]) # covariates
sim <- data[,4] # survival time
ind.cens <- data[,5] # vital status
hp.as <- data[,6] # Population (expected) mortality hazard
...
Hw <- function(t,lambda,kappa,log.p=FALSE){
...
print(kable(cbind(true.par,MLEEWG),digits=4))
...
CI <- Conf.Int(log.likewG,MLEEWG,level=0.95,index=c(1,2,3))
...
hazard.fit <- Vectorize(function(t)  hexpw(t,MLEEWG[1],MLEEWG[2],MLEEWG[3]))
...
NS0 <- Vectorize(function(t)  exp(-Hexpw(t,MLEEWG[1],MLEEWG[2],MLEEWG[3])))
NS1 <- Vectorize(function(t)  exp(-Hexpw(t*exp(MLEEWG[6]),MLEEWG[1],MLEEWG[2],MLEEWG[3])*exp(MLEEWG[9] - MLEEWG[6])))
...
legend(2.5,0.9, c("Comorb = 0","Comorb = 1"),
...
tempf <- function(par){
  par[1:3] = exp(par[1:3])
  return(log.likewG( par ))
}
r.MLE <- MLEEWG
r.MLE[1:3] <- log(MLEEWG[1:3])
HESS <- hessian(tempf,x=r.MLE)
Sigma <- solve(HESS)
...
conf.int.NS <- function(t0,level,n.boot,x.int){
  boot <- vector()
  S.par <- function(par) mean( exp(-Hexpw(t0*exp(x.int%*%par[c(4:6)]),par[1],par[2],par[3])*exp(x.int%*%(par[c(7:9)]-par[c(4:6)]))))
